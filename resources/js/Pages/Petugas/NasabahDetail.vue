        ...
        <div class="min-h-screen py-6 px-4 sm:px-6 bg-gray-50 dark:bg-gray-950">
            <div class="space-y-6">
                ...
                <header class="flex items-center justify-between mb-2">
                    <button
                        @click="$inertia.visit('/petugas/dashboard')"
                    ...
                    >
                        <div>
                            ...
                            <p class="text-sm text-gray-500 dark:text-gray-300">
                                Tanggal:
                                {{ formatDate(setoranHariIni.tanggal) }}
                            </p>
                            <p
                            ...
                            >
                                Rp
                                {{
                                    setoranHariIni.jumlah?.toLocaleString(
                                        "id-ID"
                                    )
                                }}
                            </p>
                        ...
                        <!-- Tombol Edit Nominal -->
                        <button
                            @click="openEditModal(setoranHariIni)"
                            :disabled="!canEdit"
...
// Setoran hari ini
const setoranHariIni = computed(() => props.nasabah.setorans?.[0] ?? null);

// Status request terakhir
const latestRequestStatus = computed(() => {
    const reqs = setoranHariIni.value?.requests ?? [];
...
const isSetoranValid = computed(() => Number(editJumlah.value) >= 100);
...
// Buka modal edit
function openEditModal(setoran) {
    editingSetoran.value = setoran;
    editJumlah.value = setoran.jumlah.toString();
...
function submitEdit() {
    router.post(
        route("petugas.setoran.ajukanEdit"),
        {
            setoran_id: editingSetoran.value.id,
        ...
        },
        {
            onSuccess: () => {
                ...
                notif.value = {
                    show: true,
                    message:
                        "Permintaan koreksi nominal terkirim ke supervisor!",
                };
                setTimeout(() => (notif.value.show = false), 3000);
                router.reload({ only: ["nasabah"] });
            },
        }
    );
}
...
watch(
    () => setoranHariIni.value?.requests?.slice(-1)[0]?.status,
    (newStatus, oldStatus) => {
        if (oldStatus !== "approved" && newStatus === "approved") {
            notif.value = {
                show: true,
                message:
                    "Supervisor telah menyetujui. Tombol Edit sekarang aktif!",
            };
            setTimeout(() => (notif.value.show = false), 3000);
        }
    }
