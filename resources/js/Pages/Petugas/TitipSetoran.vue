<template>
    <PetugasLayout :user="petugas" activePage="dashboard">
                ...
                >
                    ...
                    >
                        <div>
                            ...
                            <span
                                class="inline-block bg-blue-100 text-blue-800 dark:bg-blue-900 dark:text-blue-300 text-xs font-semibold px-2 py-0.5 rounded-full mt-1"
                            >
                            ...
                            <p
                                v-if="
                                    titipSetoransLocal[nasabah.id]?.status ===
                                        'sudah_setor' ||
                                    titipSetoransLocal[nasabah.id]?.status ===
        ...
        <InputSetoranModal
            ...
            @submit="submitTitipSetoranWrapper"
        />
    ...
    Object.values(newSetorans).forEach((titip) => {
        if (titip.nasabah_id) {
            titipSetoransLocal[String(titip.nasabah_id)] = {
                ...titip,
                status: titip.status ?? titip.status_titip_setoran,
                status_titip_setoran: titip.status_titip_setoran,
                titip_setoran_id: titip.titip_setoran_id ?? titip.id,
            };
        }
    });
}
...
function confirmCancel(reasonFromModal) {
    ...
    axios
        .delete(
            route("petugas.titipsetoran.destroy", {
                id: titip.titip_setoran_id,
            }),
            { data: { alasan } }
        )
        .then((res) => {
...
function isEditEnabled(id) {
    const status = titipSetoransLocal[id]?.status;
...
function isCancelEnabled(id) {
    ...
    if (!titip || !titip.titip_setoran_id) return false;
...
function goToDetail(nasabahId) {
    ...
    router.post(route("petugas.titipsetoran.storeToSession"), {
        nasabah_id: nasabahId,
    });
}
...
// Submit titip setoran
function submitTitipSetoranWrapper(value, callback) {
